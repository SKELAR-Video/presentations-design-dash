...
(() => {
  ...
  // Текст дека живе в розмітці, а не в JS-рядках. Дек-бандл, що збирає слайди
  // скриптом, виглядає так само, але його неможливо виміряти: облік змісту
  // бачить нуль, витяг слайдів ламається, і «втрачено все» стає нерозрізненним
  // від «втрачено нічого». Виміряно на живому деку: 129 із 129 елементів ТЗ
  // «зникли», поки текст діставали зі скриптів.
  const scriptCyr = [...document.scripts]
    .filter(sc => !sc.hasAttribute('data-skelar-check'))   // сам вимірювач — теж скрипт із кирилицею
    .filter(sc => /[А-Яа-яЇїІіЄєҐґ][^<]{15,}/.test(sc.textContent || '')).length;
  say2('текст у розмітці, не в скриптах', scriptCyr === 0,
    scriptCyr ? `${scriptCyr} <script> зі змістовим текстом` : 'чисто');
  if (catalogue) {
