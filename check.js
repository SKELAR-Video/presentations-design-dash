...
(() => {
  ...
  // Банер вбудованого сторожа (deck-check.js) — службовий елемент, не контент.
  ...
  document.querySelectorAll('.slide').forEach((s, i) => {
    ...
    // Міряється намальоване проти краю блоків, із просвітом 40px: 24 рятували від налізання, але читались тіснотою.
    ...
    if (HEAD) {
      ...
      if (bodies.length) t('заголовок не налазить на контент', hit === 0,
        `просвіт ${worstGap === Infinity ? '—' : Math.round(worstGap)}px (треба ≥40)`);
    }
    ...
    // Смуга контенту доходить до поля з ОБОХ боків. Перевірка «ніщо не виходить
    // за поле 100» — одностороння: недотягнутий на 12px правий край проходив
    // її ідеально. Це та сама помилка, що з кеглем: «не переповнено» я міряв
    // завжди, «не недовикористано» забував.
    // .bul — половина ширини за задумом (права половина під фото), .right
    // починається на 960; тому в них міряється лише той край, який має сенс.
    s.querySelectorAll('.band,.sheet,.cols,.subs,.rails,.tl,.right,.bul').forEach(e => {
      const b = e.getBoundingClientRect();
      const cls = e.className;
      const wantLeft = !/\bright\b/.test(cls);
      const wantRight = !/\bbul\b/.test(cls);
      if (wantLeft) t('лівий край смуги на полі 100', Math.abs(b.left - S.left - 100) <= 2,
        Math.round(b.left - S.left));
      if (wantRight) t('правий край смуги на полі 100', Math.abs(S.right - b.right - 100) <= 2,
